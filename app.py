...
MARCAP_MIN = 300_000_000_000           # 3000억
...
def analyze_stock(
    ...
    start = (
        datetime.today() - timedelta(days=140)
    ).strftime("%Y-%m-%d")
    ...
    if (
        ...
        and
        trade_amount_3avg < TRADE_AMOUNT_20AVG_MIN
    ):
        return None
...
def show_table(df, cols):
    ...
    for _, row in df.iterrows():
        ...
        with st.container(border=True):
            ...
            if (
                "original_grade" in row
                and
                pd.notna(row.get("original_grade"))
            ):

                st.caption(
                    f"원래등급: {row['original_grade']}"
                )
